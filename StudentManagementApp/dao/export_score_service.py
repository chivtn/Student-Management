...
def generate_avg_score_excel(class_id: int, year: str, semester: int = None, teacher=None):
    ...
    academic_year = classroom.academic_year if classroom else year
    ...
    setup_headers(ws, subject_name, class_name, academic_year, semester)
    populate_score_data(ws, 6, students, academic_year, semester)

    file_stream = BytesIO()
    wb.save(file_stream)
    file_stream.seek(0)

    filename = f"Bang_diem_{subject_name}_{class_name}_{academic_year}.xlsx"
    return file_stream, filename

def setup_headers(ws, subject, class_name, year, semester):
    if semester:
        merge_range = "A1:C1"
    else:
        merge_range = "A1:E1"
...
def populate_score_data(ws, header_row, students, academic_year, semester):
    ...
    for index, student in enumerate(students, 1):
        ...
        if semester:
            ...
            avg1 = calculate_avg_score(student.id, academic_year, 1)
            avg2 = calculate_avg_score(student.id, academic_year, 2)
